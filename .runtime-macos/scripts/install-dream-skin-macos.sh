...
if [ ! -f "$THEME_DIR/theme.json" ]; then
  "$SCRIPT_DIR/switch-theme-macos.sh" --id preset-endfield-frontier --no-apply >/dev/null
fi
...
if [ "$CREATE_LAUNCHERS" = "true" ]; then
  printf 'Optional Desktop launchers were created because --create-launchers was requested.\n'
else
  printf 'No Desktop files were created; use the launchers beside the downloaded installer.\n'
fi
